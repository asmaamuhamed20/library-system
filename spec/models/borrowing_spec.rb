...
RSpec.describe Borrowing, type: :model do
  ...
  describe '.create_with_params' do
    let(:valid_attributes) do
      {
        ...
        returned_at: nil  # Ensure returned_at is provided if it's required
      }
    end
    ...
    it 'does not create a new Borrowing with invalid parameters' do
      result = Borrowing.create_with_params({ user_id: nil, book_id: book.id, borrowed_at: nil, due_date: nil, returned_at: nil })
      expect(result[:status]).to eq(:unprocessable_entity)
      expect(result[:errors].full_messages).to include("User can't be blank", "Borrowed at can't be blank", "Due date can't be blank", "Returned at can't be blank")
    end
  end
  ...
  describe '#update_with_params' do
    ...
    let(:valid_update_attributes) { { returned_at: Time.current } }
  ...
  describe '#book_available' do
    ...
    it 'returns false if the book is already borrowed' do
      new_borrowing = build(:borrowing, user: user, book: book, borrowed_at: Time.current, due_date: 1.week.from_now)
